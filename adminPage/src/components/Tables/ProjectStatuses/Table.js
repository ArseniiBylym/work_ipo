...
class TableContainer extends Component {
    ...
    confirmDelete() {
		...
		if (token) {
			axios.delete(`/adminpanel/project_statuses/${this.state.DeleteItemId}`, {
				headers: {
					token: token
				}
			})
    ...
    saveChanges(item) {
		...
		if (token) {
			if (!item.id) {
				axios.post(`/adminpanel/project_statuses/`, item, {
					headers: {
						token: token
					}
				})
				...
				axios.put(`/adminpanel/project_statuses/${item.id}`, item, {
					headers: {
						token: token
					}
				})
    ...
    getTableData() {
		...
		if (token) {
			...
			axios.get(`/adminpanel/project_statuses`, {
				headers: {
					token: token
				}
			})
