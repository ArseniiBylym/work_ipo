...
class TableContainer extends Component {
    ...
    confirmDelete() {
		...
		if (token) {
			axios.delete(`/adminpanel/purchasesProjects/${this.state.DeleteItemId}`, {
				headers: {
					token: token
				}
			})
    ...
    saveChanges(item) {
		...
		if (token) {
			if (!item.id) {
				axios.post(`/adminpanel/purchasesProjects/`, item, {
					headers: {
						token: token
					}
				})
				...
				axios.put(`/adminpanel/purchasesProjects/${item.id}`, item, {
					headers: {
						token: token
					}
				})
    ...
    getTableData() {
		...
		if (token) {
			...
			axios.get(`/adminpanel/purchasesProjects`, {
				headers: {
					token: token
				}
			})
