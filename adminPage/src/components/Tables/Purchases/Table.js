...
class TableContainer extends Component {
    ...
    confirmDelete() {
		...
		if (token) {
			axios.delete(`/adminpanel/purchases/${this.state.DeleteItemId}`, {
				headers: {
					token: token
				}
			})
    ...
    saveChanges(item) {
		...
		if (token) {
			if (!item.id) {
				axios.post(`/adminpanel/purchases/`, item, {
					headers: {
						token: token
					}
				})
				...
				axios.put(`/adminpanel/purchases/${item.id}`, item, {
					headers: {
						token: token
					}
				})
    ...
    getTableData() {
		...
		if (token) {
			...
			axios.get(`/adminpanel/purchases`, {
				headers: {
					token: token
				}
			})
