...
class TableContainer extends Component {
    ...
    confirmDelete() {
		...
		if (token) {
			axios.delete(`/adminpanel/ourteam/${this.state.DeleteItemId}`, {
				headers: {
					token: token
				}
			})
    ...
    saveChanges(item) {
		...
		if (token) {
			if (!item.id) {
				axios.post(`/adminpanel/ourteam/`, item, {
					headers: {
						token: token
					}
				})
				...
				axios.put(`/adminpanel/ourteam/${item.id}`, item, {
					headers: {
						token: token
					}
				})
