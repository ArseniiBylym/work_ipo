...
class TableContainer extends Component {
    ...
    confirmDelete() {
		...
		if (token) {
			axios.delete(`/adminpanel/enterpreneurs/${this.state.DeleteItemId}`, {
				headers: {
					token: token
				}
			})
    ...
    saveChanges(item) {
		...
		if (token) {
			if (!item.id) {
				axios.post(`/adminpanel/enterpreneurs/`, item, {
					headers: {
						token: token
					}
				})
				...
				axios.put(`/adminpanel/enterpreneurs/${item.id}`, item, {
					headers: {
						token: token
					}
				})
    ...
    getTableData() {
		...
		if (token) {
			...
			axios.get(`/adminpanel/enterpreneurs`, {
				headers: {
					token: token
				}
			})
