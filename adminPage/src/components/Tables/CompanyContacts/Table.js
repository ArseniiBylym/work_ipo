...
class TableContainer extends Component {
    ...
    confirmDelete() {
		...
		if (token) {
			axios.delete(`/adminpanel/companyContacts/${this.state.DeleteItemId}`, {
				headers: {
					token: token
				}
			})
    ...
    saveChanges(item) {
		...
		if (token) {
			if (!item.id) {
				axios.post(`/adminpanel/companyContacts/`, item, {
					headers: {
						token: token
					}
				})
				...
				axios.put(`/adminpanel/companyContacts/${item.id}`, item, {
					headers: {
						token: token
					}
				})
    ...
    getTableData() {
		...
		if (token) {
			...
			axios.get(`/adminpanel/companyContacts`, {
				headers: {
					token: token
				}
			})
