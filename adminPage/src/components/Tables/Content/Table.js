...
class TableContainer extends Component {
    ...
    confirmDelete() {
		...
		if (token) {
			axios.delete(`/adminpanel/content/${this.state.DeleteItemId}`, {
				headers: {
					token: token
				}
			})
    ...
    saveChanges(item) {
		...
		if (token) {
			if (!item.id) {
				axios.post(`/adminpanel/content/`, item, {
					headers: {
						token: token
					}
				})
				...
				axios.put(`/adminpanel/content/${item.id}`, item, {
					headers: {
						token: token
					}
				})
